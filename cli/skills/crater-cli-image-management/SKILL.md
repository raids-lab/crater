...
version: 1.0.0
description: "Crater CLI 用户镜像与环境管理：指导 AI Agent 使用 crater image 构建、上传、删除、分享、更新自己可管理的镜像、管理 CUDA base image 和 Harbor 凭据。管理员镜像操作请使用 crater-cli-admin-image-management。"
metadata:
  requires:
    bins: ["crater"]
...
crater image cuda add --image-label cuda124 --label "CUDA 12.4" --value registry/nvidia/cuda:12.4 --json --no-interactive
...
6. 如果用户要求平台级镜像列表、删除他人镜像、修改公共可见性，切换到管理员 skill。
