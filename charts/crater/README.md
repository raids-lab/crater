...
| backendConfig.smtp | object | `{"enable":false,"host":"mail.example.com","insecureSkipVerify":false,"notify":"example@example.com","password":"<MASKED>","port":25,"user":"example"}` | Configuration for email notifications via SMTP If Enable is false, email notifications will be disabled |
...
| backendConfig.smtp.insecureSkipVerify | bool | `false` | Skip SMTP TLS certificate and hostname verification Insecure: Enable only for servers with untrusted self-signed certificates |
...
## Kthena inference permissions

The backend service account can manage Kthena ModelBooster, ModelServing, ModelServer, and ModelRoute resources and use `services/proxy` to forward inference requests through the Kubernetes API. Install the matching Kthena controllers and CRDs separately. The model-deployment feature is disabled by default; an administrator enables it in platform settings after validating the Kthena installation. Disabling the feature hides the entry points and rejects inference API access; it does not delete existing deployments or remove RBAC permissions.
