...
| agent | object | `{"allowLocalLLMConfigFallback":false,"defaultOrchestrationMode":"single_agent","enabled":true,"env":[],"internalToken":"<MUSTEDIT>","platformRuntime":{"kubernetes":{"kubectlBin":"kubectl","namespace":"crater-workspace","safety":{"blockedCommandPatterns":["delete namespace","delete node","delete pv","delete crd","cluster-info dump","auth can-i","--as=","exec -it","port-forward","proxy","apply -f http"],"protectedNamespaces":["kube-system","kube-public","kube-node-lease"]},"timeoutSeconds":30},"toolRouting":{"localCoreTools":[],"localWriteTools":[]},"webSearch":{"allowedDomains":[],"enabled":false,"maxContentChars":4000,"timeoutSeconds":10}},"port":8000,"replicaCount":1,"resources":{"limits":{"cpu":"1000m","memory":"2Gi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"port":80,"type":"ClusterIP"}}` | Crater Agent Python service configuration |
| agent.allowLocalLLMConfigFallback | bool | `false` | Whether Python Agent may fall back to its local llm-clients.json |
| agent.defaultOrchestrationMode | string | `"single_agent"` | Default orchestration mode ("single_agent" or "multi_agent") |
| agent.enabled | bool | `true` | Whether to deploy the Crater Agent service with the platform |
| agent.env | list | `[]` | Additional environment variables for the Agent container |
| agent.internalToken | string | `"<MUSTEDIT>"` | Shared internal token for backend <-> agent service calls; replace in production |
| agent.platformRuntime | object | `{"kubernetes":{"kubectlBin":"kubectl","namespace":"crater-workspace","safety":{"blockedCommandPatterns":["delete namespace","delete node","delete pv","delete crd","cluster-info dump","auth can-i","--as=","exec -it","port-forward","proxy","apply -f http"],"protectedNamespaces":["kube-system","kube-public","kube-node-lease"]},"timeoutSeconds":30},"toolRouting":{"localCoreTools":[],"localWriteTools":[]},"webSearch":{"allowedDomains":[],"enabled":false,"maxContentChars":4000,"timeoutSeconds":10}}` | Agent-side runtime config for local diagnostic tools only |
| agent.port | int | `8000` | Agent container port |
| agent.replicaCount | int | `1` | Number of Agent replicas |
| agent.resources | object | `{"limits":{"cpu":"1000m","memory":"2Gi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Agent resource requests and limits |
| agent.service.port | int | `80` | Agent service port |
| agent.service.type | string | `"ClusterIP"` | Agent service type |
| backendConfig | object | `{"agent":{"approvalHook":{"circuitBreakerCooldownSeconds":60,"circuitBreakerThreshold":5,"enabled":true,"maxConcurrent":3,"maxPerMinute":10,"totalTimeoutSeconds":60},"ops":{"kubernetes":{"context":"","kubectlBin":"kubectl","kubeconfigPath":"","namespace":"crater-workspace","timeoutSeconds":30},"webSearch":{"allowedDomains":[],"enabled":false,"timeoutSeconds":10}},"serviceURL":"http://crater-agent-svc"},"auth":{"ldap":{"alias":"","attributeMapping":{"displayName":"cn","email":"mail","username":"uid"},"enable":false,"help":"","server":{"address":"ldap://ldap.example.com:389","baseDN":"dc=example,dc=org","bindDN":"cn=admin,dc=example,dc=org","bindPassword":"<MUSTEDIT>"},"uid":{"ldapAttribute":{"gid":"gidNumber","uid":"uidNumber"},"rid":{"offset":10000,"pgidAttribute":"primaryGroupID","sidAttribute":"objectSid"},"source":"default"}},"normal":{"allowLogin":true,"allowRegister":true},"token":{"accessTokenSecret":"example-access-token","refreshTokenSecret":"example-refresh-token"}},"enableLeaderElection":false,"modelDownload":{"image":"crater-harbor.act.buaa.edu.cn/docker.io/library/python:3.11-slim"},"port":":8088","postgres":{"TimeZone":"Asia/Shanghai","dbname":"postgres","host":"crater-postgresql.crater-system.svc.cluster.local","password":"<MUSTEDIT>","port":5432,"sslmode":"disable","user":"postgres"},"prometheusAPI":"http://192.168.0.1:12345","registry":{"buildTools":{"proxyConfig":{"httpProxy":null,"httpsProxy":null,"noProxy":null}},"enable":false,"harbor":{"password":"<MASKED>","server":"harbor.example.com","user":"admin"}},"secrets":{"imagePullSecretName":"","tlsForwardSecretName":"crater-tls-forward-secret","tlsSecretName":"crater-tls-secret"},"smtp":{"enable":false,"host":"mail.example.com","notify":"example@example.com","password":"<MASKED>","port":25,"user":"example"},"storage":{"prefix":{"account":"accounts","public":"public","user":"users"},"pvc":{"readOnlyMany":null,"readWriteMany":"crater-rw-storage"}}}` | Backend configuration |
| backendConfig.agent | object | `{"approvalHook":{"circuitBreakerCooldownSeconds":60,"circuitBreakerThreshold":5,"enabled":true,"maxConcurrent":3,"maxPerMinute":10,"totalTimeoutSeconds":60},"ops":{"kubernetes":{"context":"","kubectlBin":"kubectl","kubeconfigPath":"","namespace":"crater-workspace","timeoutSeconds":30},"webSearch":{"allowedDomains":[],"enabled":false,"timeoutSeconds":10}},"serviceURL":"http://crater-agent-svc"}` | Crater Agent service and backend-side ops tool policy |
| backendConfig.agent.approvalHook.enabled | bool | `true` | Whether to use the Agent to evaluate approval orders |
| backendConfig.agent.serviceURL | string | `"http://crater-agent-svc"` | Base URL of the Python Agent service; keep cluster-internal |
...
| backendConfig.auth.ldap.help | string | `""` | Description shown in a tooltip for this auth method If empty, frontend shows a default description. |
...
| backendConfig.auth.ldap.uid | object | `{"ldapAttribute":{"gid":"gidNumber","uid":"uidNumber"},"rid":{"offset":10000,"pgidAttribute":"primaryGroupID","sidAttribute":"objectSid"},"source":"default"}` | UID/GID acquisition settings |
| backendConfig.auth.ldap.uid.ldapAttribute | object | `{"gid":"gidNumber","uid":"uidNumber"}` | LDAP attribute names for UID/GID (used when source is "ldap") |
| backendConfig.auth.ldap.uid.rid | object | `{"offset":10000,"pgidAttribute":"primaryGroupID","sidAttribute":"objectSid"}` | RID-based calculation settings (used when source is "rid") |
| backendConfig.auth.ldap.uid.rid.offset | int | `10000` | Offset for UID/GID calculation (e.g., UID = RID + 10000) |
| backendConfig.auth.ldap.uid.rid.pgidAttribute | string | `"primaryGroupID"` | LDAP attribute storing the primary group RID (defaults to "primaryGroupID") |
| backendConfig.auth.ldap.uid.rid.sidAttribute | string | `"objectSid"` | LDAP attribute storing the binary SID (defaults to "objectSid") |
| backendConfig.auth.ldap.uid.source | string | `"default"` | Source of UID/GID ("default", "ldap", "rid", "external") - "rid" (Recommended): Calculates UID/GID from LDAP objectSid using RID + offset. - "ldap": Fetches UID/GID from specified LDAP attributes. - "external" (Deprecated): Fetches UID/GID from an external HTTP service. - "default" / "none": Uses default UID=1001, GID=1001. |
