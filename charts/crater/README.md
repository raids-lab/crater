...
| backendConfig.checkpointScanner | object | `{"endpoint":"","timeoutSeconds":30}` | Always-on checkpoint scanner service used by backend checkpoint scan APIs |
| backendConfig.checkpointScanner.endpoint | string | `""` | HTTP base URL for the scanner service. If empty, helper templates fill the in-cluster service endpoint. |
| backendConfig.checkpointScanner.timeoutSeconds | int | `30` | Backend-to-scanner request timeout in seconds |
...
| checkpointScanner | object | `{"concurrency":4,"enabled":true,"port":7330,"replicas":1,"resources":{"limits":{"cpu":"500m","memory":"512Mi"},"requests":{"cpu":"50m","memory":"64Mi"}}}` | Checkpoint scanner service configuration |
| checkpointScanner.concurrency | int | `4` | Max concurrent scan requests handled by one scanner pod |
| checkpointScanner.enabled | bool | `true` | Whether to deploy the always-on checkpoint scanner service |
| checkpointScanner.port | int | `7330` | HTTP port exposed by the scanner service |
| checkpointScanner.replicas | int | `1` | Number of checkpoint scanner replicas |
| checkpointScanner.resources | object | `{"limits":{"cpu":"500m","memory":"512Mi"},"requests":{"cpu":"50m","memory":"64Mi"}}` | Resource requests and limits for checkpoint scanner pods |
