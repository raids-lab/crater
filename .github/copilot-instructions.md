...
- **用户需求与体验**: 功能应服务真实用户需求。设计行为、错误信息、操作流程、默认值和文档时，应持续考虑平台用户、管理员和贡献者的实际使用体验。
...
- **提交 / 推送前人工检查**: 最终提交、推送或创建 PR 描述前，必须要求开发者提供亲自执行的人工检查结果。AI 可以辅助列出应打开的页面、检查的功能、运行的命令或需阅读的文档；文档改动必须由开发者人工阅读检查，不能只依赖 AI 生成或 AI 自检。
- **PR 创建前确认**: AI 可以协助整理 PR 描述并在工具可用时创建 PR，但必须先把最终 PR 描述展示给开发者并取得确认；不能把开发者没看过的 AI 生成文本直接用于 PR。
- **PR 后续迭代**: 创建 PR 后提醒开发者检查 workflow 状态。处理 Copilot review 或人工 review 时，先判断意见是否正确、是否值得修改，并提出方案；未经开发者讨论和确认，不要直接修改代码。
...
- **人工检查校验**: 评估 PR 描述是否区分 AI / 自动化检查与开发者人工检查；文档改动若缺少开发者人工阅读检查，应提醒补齐。
...
Crater 是基于 Kubernetes 的异构（GPU）算力管理平台，monorepo 含 `backend/`（Go 服务，含 `internal/storage/`）、`frontend/`（React 19 + TypeScript）、`cli/`、`website/`（Fumadocs 文档站）、`charts/`（Helm）等。业务定位与架构见各 `README`。

## 文档来源 (Documentation Sources)

开发与贡献的**完整权威规范**是各级 `CONTRIBUTING`，本文件与各 `*.instructions.md` 均**引用**它，不另立或完整重复其规则：

- 全局守则、流程、Commit / PR 规范：根 [`CONTRIBUTING.md`](../CONTRIBUTING.md)
- 各模块规范：`backend/`、`frontend/`、`website/`、`cli/`、`charts/` 各自的 `CONTRIBUTING.md`（CLI 经 `cli/docs/*` 承载契约）

`*.instructions.md` 的职责是为审查固化最重要的 MUST/SHOULD 拦截项与审查特有补充，并指明完整规则去 CONTRIBUTING 查阅；重要规则允许重复。`README` 仅面向使用者，不承载开发规范。

新增或修改开发约定时，也遵循这一职责划分：影响最终代码、文档、API、UI、Chart 或测试结果的规则先写入合适的 `CONTRIBUTING`；偏 AI 开发过程的规则可写入 `skills/`；审查指令与 Skill 引用权威文档，并允许重复重要约定。
