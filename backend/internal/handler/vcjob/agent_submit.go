...
func (s *agentJobSubmitter) SubmitJupyterJob(
	...
	if err := s.preCheckJobCreate(ctx, token, scheduleType, true); err != nil {
		return nil, err
	}

	if err := vcqueue.EnsureAccountQueueExists(ctx, s.mgr.client, token, token.AccountID); err != nil {
		return nil, agentSubmitErrorf("failed to ensure account queue exists: %v", err)
	}
	if err := vcqueue.EnsureUserQueueExists(ctx, s.mgr.client, token, token.AccountID, token.UserID); err != nil {
		return nil, agentSubmitErrorf("failed to ensure user queue exists: %v", err)
	}

	if req.AlertEnabled && !utils.CheckUserEmail(ctx, token.UserID) {
		return nil, agentSubmitErrorf("email not verified")
	}
...
func (s *agentJobSubmitter) SubmitTrainingJob(
	...
	if err := s.preCheckJobCreate(ctx, token, scheduleType, false); err != nil {
		return nil, err
	}
...
func normalizeAgentSubmitGPUResourceName(current v1.ResourceName, gpuModel string) v1.ResourceName {
	model := normalizeAgentSubmitGPUModelName(gpuModel)
	if model == "" {
		return current
	}
