...
func (mgr *AgentMgr) toolCreateImageBuild(c *gin.Context, token internalutil.JWTMessage, rawArgs json.RawMessage) (any, error) {
	...
	case agentImageBuildModePipApt:
		...
		return mgr.buildSubmittedImageBuildResult(c, imagePackName, imageLink, string(mode), model.PipApt, archs), nil
	case agentImageBuildModeDocker:
		...
		return mgr.buildSubmittedImageBuildResult(c, imagePackName, imageLink, string(mode), model.Dockerfile, archs), nil
	case agentImageBuildModeEnvd:
		...
		enableJupyter := getToolArgBool(args, "enable_jupyter")
		enableZsh := getToolArgBool(args, "enable_zsh")
		...
		return mgr.buildSubmittedImageBuildResult(c, imagePackName, imageLink, string(mode), model.EnvdAdvanced, archs), nil
	case agentImageBuildModeEnvdRaw:
		...
		return mgr.buildSubmittedImageBuildResult(c, imagePackName, imageLink, string(mode), model.EnvdRaw, archs), nil
	default:
