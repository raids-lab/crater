...
type Response[T any] struct {
	...
	MessageKey string    `json:"msgKey,omitempty"`
}
...
func wrapResponse(c *gin.Context, msg, msgKey string, data any, code ErrorCode) {
...
func ErrorWithKey(c *gin.Context, msgKey, msg string, errorCode ErrorCode) {
	wrapResponse(c, msg, msgKey, nil, errorCode)
}
...
func HTTPError(c *gin.Context, httpCode int, err string, errorCode ErrorCode) {
	HTTPErrorWithKey(c, httpCode, "", err, errorCode)
}

func HTTPErrorWithKey(c *gin.Context, httpCode int, msgKey, err string, errorCode ErrorCode) {
	c.JSON(httpCode, gin.H{
		"code":   errorCode,
		"data":   nil,
		"msg":    err,
