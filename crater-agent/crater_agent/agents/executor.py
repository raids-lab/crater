    ...
    def build_tool_loop_prompts(
        ...
        *,
        ...
        system_prompt = (
                ...
                + "\n\n"
                "执行要求：\n"
                "- 当前用户角色不是管理员时，不要尝试执行管理员专属写操作；应直接说明“你当前没有管理员权限，不能执行该运维操作，如需处理请联系管理员”，不要说成当前会话、认证服务或临时故障。\n"
                "- 纯查询/诊断请求不要擅自执行写操作。\n"
                ...
                "- 写意图、目标对象、权限边界和最小风险事实明确时，必须首轮调用真实确认工具进入 confirmation_required；不要先让用户再回复“确认”，不要输出伪工具文本、手写 JSON 工具块或仅给用户泛化操作建议。\n"
                ...
                "- 对 stop/resubmit/delete：如果用户要求处理单个作业，目标 jobName 必须明确；目标明确且只读核验通过后应直接发起确认卡片，不要再文字询问是否确认；如果只读核验返回 not_found、permission denied、403 或无权访问，应直接说明“该作业不存在或你没有访问权限”，不得展示作业详情或发起确认动作。\n"
                ...
                "- 对 create_jupyter_job / create_webide_job / create_custom_job / create_pytorch_job / create_tensorflow_job：优先复用模板、配额、镜像和用户给定参数；缺关键字段时可以生成需要用户确认/补全的提交动作，但不要凭空捏造镜像或资源。\n"
        ...
        *,
        ...
        result = await self.run_json(
            system_prompt=(
                ...
                "- 当前用户角色不是管理员时，不要输出管理员专属写操作；应返回空 actions，由最终回答明确说明用户没有管理员权限并建议联系管理员。\n"
                "- 只有用户明确要求操作时才选择写操作。\n"
                "- 纯诊断/查询请求不应产生写操作。\n"
                '- "帮我看看这个作业" ≠ "帮我停止这个作业"。\n'
                ...
                "- stop/resubmit/delete/create/scale/restart/node isolation 都必须由你根据用户输入、页面上下文、证据和工具描述选择具体工具及参数；信息足够且有权限时应直接输出对应 action 触发确认卡片，不要依赖外部 controller 替你补默认参数，也不要先要求用户口头确认。\n"
                "- 对 stop/resubmit/delete，如果证据表明目标作业 not_found、permission denied、403 或无权访问，actions 必须为空；不要用确认卡片泄露或操作他人作业。\n"
