...
from orchestrators.artifacts import RoutingDecision, RoutingTargets
        ...
        *,
        ...
        result = await self.coordinator_agent.run_json(
            system_prompt=(
                ...
                "- 只有明确纯文本闲聊、平台介绍、无需任何平台事实或工具的简单问题，才可判为 simple/help。\n"
                "- 凡是涉及作业、集群、节点、队列、日志、指标、权限、提交、确认、续接或运维动作，都必须判为 agent；不要走纯文本 fast path。\n"
                "- 若用户输入同时包含确认/执行/继续与新的追问，不要把整轮当成纯确认；确认动作回流后，仍要保留并处理同一条消息里的追问部分。\n"
