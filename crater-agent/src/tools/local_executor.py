...
class LocalToolExecutor:
    ...
    async def _handle_web_search(self, tool_args: dict[str, Any]) -> dict[str, Any]:
        ...
        from tools.camel_tools import camel_web_search  # noqa: PLC0415
    ...
    async def _handle_execute_code(self, tool_args: dict[str, Any]) -> dict[str, Any]:
        ...
        from tools.camel_tools import camel_execute_code  # noqa: PLC0415
    ...
    async def _handle_get_node_kernel_diagnostics(self, tool_args: dict[str, Any]) -> dict[str, Any]:
        ...
        dmesg_lines_list = [
            line.strip()
            for line in sections.get("DMESG", "").splitlines()
            if line.strip() and line.strip() != "(no matches)"
        ]
        d_state_lines = [
            line.strip()
            for line in sections.get("D_STATE", "").splitlines()
    ...
    async def _handle_get_node_gpu_info(self, tool_args: dict[str, Any]) -> dict[str, Any]:
        ...
        if procs_text and not procs_text.startswith("("):
            gpu_processes = [line.strip() for line in procs_text.splitlines() if line.strip()]

        modules_text = sections.get("NVIDIA_MODULES", "").strip()
        nvidia_modules: list[str] = []
        if modules_text and not modules_text.startswith("("):
            nvidia_modules = [line.strip() for line in modules_text.splitlines() if line.strip()]
    ...
    async def _handle_get_node_accelerator_info(self, tool_args: dict[str, Any]) -> dict[str, Any]:
        ...
        pcie_devices = [
            line.strip() for line in pcie_text.splitlines() if line.strip()
        ] if not pcie_text.startswith("(") else []

        # Parse kernel modules
        modules_text = sections.get("ACCEL_MODULES", "").strip()
        accel_modules = [
            line.strip() for line in modules_text.splitlines() if line.strip()
        ] if not modules_text.startswith("(") else []
    ...
    async def _handle_k8s_top_nodes(self, tool_args: dict[str, Any]) -> dict[str, Any]:
        ...
        lines = [line for line in result.get("content", "").splitlines() if line.strip()]
        nodes = []
        for line in lines:
            parts = line.split()
            if len(parts) >= 5:
