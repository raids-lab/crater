...
Mops 是驱动 Crater 智算平台的多智能体运维框架。它提供了一套可组合的架构，用于构建和编排基于 LLM 的智能体，这些智能体可在 GPU/加速器集群上执行故障诊断、资源优化、主动巡检和自动化运维。
...
2. 添加到 `AUTO_TOOLS`（只读）、预留的 `AUTO_ACTION_TOOLS`（system-only 无 HITL 副作用动作）或 `CONFIRM_TOOLS`（需确认写操作/外部动作）
...
- [image-authoring-testing.md](image-authoring-testing.md) — 镜像创建 / 导入 / 分享工作流的自然语言与离线回放测试用例
