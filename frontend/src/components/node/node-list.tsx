    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (status === NodeStatus.Occupied) {
          // 已占用：显示 occupiedReason（如果存在），否则默认文案
          tooltipContent = occupiedReason || '该节点处于账户独占状态'
        } else if (isUnschedulable) {
          // 不可调度：显示 unschedulableReason（如果存在），否则默认文案
          tooltipContent = unschedulableReason || '该节点被标记为不可调度'
        } else {
          // 运行中：显示作业数或通用文案
