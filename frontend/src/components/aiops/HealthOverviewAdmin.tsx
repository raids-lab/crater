...
import { apiGetHealthOverviewAdmin } from '@/services/api/aiops'
...
export function HealthOverviewAdmin() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiGetHealthOverviewAdmin(days)
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <PageTitle title={t('aiops.page.title')} description={t('aiops.page.loadingAdmin')} />
        <Card>
  ...
  if (error || !data) {
    return (
      <div className="space-y-4">
        <PageTitle title={t('aiops.page.title')} description={t('aiops.page.loadFailedAdmin')} />
