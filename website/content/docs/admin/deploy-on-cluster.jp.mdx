  ...
  auth:
    ldap:
      ...
      attributeMapping:
        ...
        email: "mail"
      uid:
        source: "default"  # 選択肢: "default", "ldap", "external"
```
...
`uid.source` 設定項目は、LDAP 属性または外部マイクロサービスからユーザーの UID/GID を取得することを可能にします。この機能は、主に ACT ラボ内部で長年使用されてきたストレージシステムの権限との互換性を保つために設計されました。外部クラスターの場合、`uid.source` を **`default`** に設定することを強く推奨します。プラットフォームはすべてのユーザーに対して **1001** をコンテナ内の実行 ID として統一的に使用します。これは、ほとんどの標準的な NFS やローカルストレージ環境において最もシンプルな選択肢です。
