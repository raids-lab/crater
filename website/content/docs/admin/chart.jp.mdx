...
| backendConfig.auth.ldap.uid.rid.offset | int | `10000` | source が `rid` の場合の UID 計算オフセット |
| backendConfig.auth.ldap.uid.rid.sidAttribute | string | `"objectSid"` | source が `rid` の場合、バイナリ SID を格納する LDAP 属性名 |
