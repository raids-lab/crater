  ...
  auth:
    ldap:
      ...
      uid:
        ...
        # - "rid": Parse RID from objectSid and compute UID/GID as RID + offset (for AD + Winbind style setups)
        # - "external": Use a legacy internal UID service (deprecated)
        source: "default"
        # When source is "rid", align with Winbind by computing UID/GID = RID + offset
        rid:
          offset: 10000
