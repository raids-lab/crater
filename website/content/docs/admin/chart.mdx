...
| backendConfig.auth.ldap.uid.rid.offset | int | `10000` | 当 source 为 `rid` 时的 UID 计算偏移量 |
| backendConfig.auth.ldap.uid.rid.sidAttribute | string | `"objectSid"` | 当 source 为 `rid` 时，存储二进制 SID 的 LDAP 属性名 |
