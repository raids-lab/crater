...
| backendConfig.auth.ldap.uid.rid.offset | int | `10000` | source가 `rid`일 때의 UID 계산 오프셋 |
| backendConfig.auth.ldap.uid.rid.sidAttribute | string | `"objectSid"` | source가 `rid`일 때 바이너리 SID를 저장하는 LDAP 속성 이름 |
