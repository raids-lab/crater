...
| backendConfig.modelMetadata.logoAllowedHosts | list | `["huggingface.co", "cdn-avatars.huggingface.co", "resouces.modelscope.cn", "resources.modelscope.cn"]` | Logo 가져오기와 캐시를 허용하는 정확한 HTTPS 호스트 목록. 리디렉션의 각 단계도 검증함 |
...
| backendConfig.registry.buildTools.proxyConfig.httpProxy | string | `null` | 이미지 빌드와 Hugging Face 다운로드 Job에서 사용하는 HTTP 송신 프록시 |
| backendConfig.registry.buildTools.proxyConfig.httpsProxy | string | `null` | 이미지 빌드와 Hugging Face 다운로드 Job에서 사용하는 HTTPS 송신 프록시 |
| backendConfig.registry.buildTools.proxyConfig.noProxy | string | `null` | 이미지 빌드와 Hugging Face 다운로드 Job이 프록시를 우회할 주소 목록 (쉼표로 구분) |
