...
| backendConfig.auth.ldap.uid.rid.offset | int | `10000` | UID calculation offset when source is `rid` |
| backendConfig.auth.ldap.uid.rid.sidAttribute | string | `"objectSid"` | LDAP attribute storing the binary SID when source is `rid` |
