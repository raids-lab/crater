  ...
  auth:
    ldap:
      ...
      attributeMapping:
        ...
        email: "mail"
      uid:
        source: "default"  # 可选: "default", "ldap", "external"
```
...
配置项 `uid.source` 允许从 LDAP 属性或外部微服务获取用户的 UID/GID。此功能主要用于兼容 ACT 实验室内部原有的分布式存储系统权限。对于外部集群，我们建议将 `uid.source` 设置为 **`default`**。平台将统一使用 **1001** 作为用户的容器内运行标识，这对于大多数标准 NFS 或本地存储环境是最简便的选择。

更详细的说明可以参阅 [配置说明](/zh/docs/admin/chart/) 文档。
