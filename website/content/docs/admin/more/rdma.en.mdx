...
3.  **Plugin Activation and Resource Exposure (RDMA Shared Device Plugin Pod)**: Once `mofed.wait` becomes `false`, the locked RDMA Shared Device Plugin Pod will start its detection logic, identify physical network cards, and report resources such as `rdma/rdma_v100` to the Kubelet.
...
#### Verification Steps
1.  **Observe Pod Status**: Run `kubectl get pods -n nvidia-network-operator -w` to confirm that `mofed-driver` and `rdma-shared-dp` Pods successfully start on the corresponding node.
2.  **Check Node Status**: Run `kubectl get node [node-name] -L network.nvidia.com/operator.mofed.wait` to confirm that the label changes back to `false`.
3.  **Confirm Resource Reporting**: Run `kubectl describe node [node-name]`. You should see `rdma/rdma_v100` again in the `Allocatable` list.
