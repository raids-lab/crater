...
By default, the core Crater Chart creates a shared volume through a StorageClass named `nfs` and mounts the same PVC into multiple storage-server replicas to store and serve user files. When a user starts a Jupyter, WebIDE, or training workload, Crater mounts the user's HOME directory into the container. Account or public files, datasets, and models selected by the user are also mounted into the workload container from this storage capability. These mounts use Crater's configured RWX shared PVC or an optional read-only PVC backed by the same underlying storage.
...
- [OpenEBS Deployment Guide](./openebs)
- [CloudNativePG Deployment Guide](./cloudnative-pg)
- [Prometheus Stack Deployment Guide](./prometheus)
- [Rook-Ceph Learning Guide](../learning/ceph/ceph-basic)
