  ...
  auth:
    ldap:
      ...
      attributeMapping:
        ...
        email: "mail"
      uid:
        source: "default"  # 선택 가능: "default", "ldap", "external"
```
...
`uid.source` 설정 항목은 LDAP 속성 또는 외부 마이크로서비스에서 사용자의 UID/GID를 가져올 수 있게 합니다. 이 기능은 주로 ACT 실험실 내부에서 기존에 사용하던 스토리지 시스템 권한과의 호환성을 위해 설계되었습니다. 외부 클러스터의 경우 `uid.source`를 **`default`**로 설정할 것을 강력히 권장합니다. 플랫폼은 모든 사용자에 대해 컨테이너 내 실행 ID를 **1001**로 통일하여 사용하며, 이는 대부분의 표준 NFS 또는 로컬 스토리지 환경에서 가장 간편한 선택입니다.
