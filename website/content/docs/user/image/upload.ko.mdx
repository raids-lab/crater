---
title: 로컬 이미지 업로드 ⬆️
description: 본 문서는 플랫폼을 통해 Harbor 에 대한 액세스 자격 증명을 얻고, 로컬 Docker 이미지를 Harbor 저장소에 업로드하는 방법을 안내합니다.
---
...
먼저, 사용자의 플랫폼 (예: Kubernetes 클러스터 관리 플랫폼, CI/CD 플랫폼 등) 에 로그인하고, Harbor 와 통합된 부분을 찾습니다.
...
얻은 자격 증명을 사용하여 Harbor 에 로그인합니다:

```bash
docker login harbor.example.com -u <사용자 이름> -p <비밀번호>
```
...
각 사용자의 Project Name 은 `user-{ACT 사용자 이름}`입니다. 💡

<Callout type="warn" title="태그 규칙">
`latest` 이외의 태그가 가리키는 이미지는 업데이트하지 마세요. 즉, 이미 `v1.0.0` 같은 태그로 Harbor 에 푸시한 이미지는 아주 작은 변경이라도 새 태그로 푸시해야 합니다. 빠르게 반복 수정이 필요하면 `latest` 태그를 직접 사용하세요. 이 규칙을 지키지 않으면 플랫폼에서 사용 중인 이미지가 최신임을 보장할 수 없습니다.
</Callout>

### ⬆️ 2.2 이미지 Harbor 로 푸시
...
이미지를 Harbor 저장소에 업로드한 후, 작업 등에서 사용하려면 플랫폼에 이미지를 가져와야 합니다. [이미지 목록 - 이미지 가져오기](/ko/docs/user/image/imageList/#import-image) 문서에 따라 업로드한 이미지를 플랫폼으로 가져오세요.
...
- Harbor URL 이 올바르고 네트워크로 접근 가능한지 확인하세요.
