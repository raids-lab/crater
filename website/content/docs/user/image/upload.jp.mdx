...
```bash
docker login harbor.example.com -u <ユーザー名> -p <パスワード>
```
...
<Callout type="warn" title="タグの取り決めについて">
`latest` 以外のタグが指すイメージは更新しないでください。つまり、すでに `v1.0.0` などのタグで Harbor にプッシュしたイメージは、ごく小さな変更でも新しいタグでプッシュしてください。迅速に反復する必要がある場合は、`latest` タグを直接使用してください。この取り決めに従わない場合、プラットフォームは使用中のイメージが最新であることを保証できません。
</Callout>
...
イメージを Harbor レポジトリにアップロードした後、ジョブなどのシナリオで使用するには、プラットフォームにイメージをインポートする必要があります。[イメージリスト - イメージのインポート](/jp/docs/user/image/imageList/#import-image) のドキュメントに従って、アップロードしたイメージをプラットフォームにインポートしてください。
