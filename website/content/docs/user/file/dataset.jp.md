...
各データセットには基本情報と利用可能な操作が表示されます。データセット詳細の「ファイル」タブでは、ファイルを閲覧し、ジョブでマウントできる共有ストレージの論理パスをコピーできます。クラスター固有の物理ディレクトリ接頭辞は表示されません。過去のダウンロードではソースとリビジョンを含む長いパスが表示される場合があります。既存ジョブがそのパスを使用している可能性があるため、Crater はそれらのディレクトリを自動移動しません。
...
同じリポジトリ ID については 1 つの公開リソースを使用します。Ready、保留中、ダウンロード中、または一時停止中の記録とその実際のパスを再利用し、申請した各ユーザーを 1 回だけ関連付けます。参照数は関連ユーザー数を表します。有効な記録がない場合は、失敗した記録を再試行するか、同名リソースが存在する別のソースから再度申請できます。
